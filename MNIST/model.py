...
class GMGANs():
    def __init__(
        ...
        self.gen_W1 = tf.Variable(tf.truncated_normal([dim_latent_codes, dim_W1*7*7], stddev=0.002), name='gen_W1')
        self.gen_bn_g1 = tf.Variable( tf.truncated_normal([dim_W1*7*7], mean=1.0, stddev=0.002), name='gen_bn_g1')
        self.gen_bn_b1 = tf.Variable( tf.zeros([dim_W1*7*7]), name='gen_bn_b1')

        self.gen_W2 = tf.Variable(tf.truncated_normal([5,5,dim_W2, dim_W1], stddev=0.002), name='gen_W2')
        ...
        self.discrim_W1 = tf.Variable(tf.truncated_normal([5,5,dim_W6,dim_W4], stddev=0.02), name='discrim_W1')
        ...
        self.discrim_W5 = tf.Variable(tf.truncated_normal([7*7*dim_W1,1], stddev=0.02), name='discrim_W5')
        
        self.aux_q_W1 = tf.Variable(tf.truncated_normal([5,5,dim_W6,dim_W4], stddev=0.02), name='aux_q_W1')
        ...
        self.aux_q_W2 = tf.Variable(tf.truncated_normal([5,5,dim_W4,dim_W3], stddev=0.02), name='aux_q_W2')
        ...
        self.aux_q_W3 = tf.Variable(tf.truncated_normal([5,5,dim_W3,dim_W2], stddev=0.02), name='aux_q_W3')
        ...
        self.aux_q_W4 = tf.Variable(tf.truncated_normal([5,5,dim_W2,dim_W1], stddev=0.02), name='aux_q_W4')
        ...
        self.aux_q_W_mean = tf.Variable(tf.truncated_normal([7*7*dim_W1,dim_latent_codes], stddev=0.02), name='aux_q_W_mean')
        self.aux_q_W_sigma= tf.Variable(tf.truncated_normal([7*7*dim_W1,dim_latent_codes], stddev=0.02), name='aux_q_W_sigma')
    ...
    def build_model(self):
        ...
        real_latent_code_mean, aux_latent_code_log_sigma=self.latent_encoder(image_real)
        ...
        gen_cost=tf.reduce_mean(p_gen)+0.0005*aux_G_cost
        
        return Y, latent_codes, component_list, image_real, discrim_cost, gen_cost, aux_G_cost, aux_latent_code_mean, aux_latent_code_log_sigma

    def discriminate(self, image):
        h1 = lrelu( tf.nn.conv2d( image, self.discrim_W1, strides=[1,2,2,1], padding='SAME' ))
        
        h2 = lrelu( batchnormalize( tf.nn.conv2d( h1, self.discrim_W2, strides=[1,1,1,1], padding='SAME'), g=self.discrim_bn_g2, b=self.discrim_bn_b2) )
        ...
        h4 = lrelu( batchnormalize( tf.nn.conv2d( h3, self.discrim_W4, strides=[1,1,1,1], padding='SAME'), g=self.discrim_bn_g4, b=self.discrim_bn_b4) )
    ...
    def latent_encoder(self, image):
        ...
        h2 = tf.nn.relu( batchnormalize( tf.nn.conv2d( h1, self.aux_q_W2, strides=[1,1,1,1], padding='SAME'), g=self.aux_q_bn_g2, b=self.aux_q_bn_b2) )
        ...
        h4 = tf.nn.relu( batchnormalize( tf.nn.conv2d( h3, self.aux_q_W4, strides=[1,1,1,1], padding='SAME'), g=self.aux_q_bn_g4, b=self.aux_q_bn_b4) )
    ...
    def generate(self, Z):
        ...
        output_shape_l4 = [self.batch_size,14,14,self.dim_W4]
        ...
        output_shape_l5 = [self.batch_size,28,28,self.dim_W5]
        ...
        output_shape_l6 = [self.batch_size,28,28,self.dim_W6]
        h5 = tf.nn.conv2d_transpose(h5, self.gen_W6, output_shape=output_shape_l6, strides=[1,1,1,1])
    ...
    def samples_generator(self, batch_size):
        ...
        output_shape_l6 = [batch_size,28,28,self.dim_W6]
        h5 = tf.nn.conv2d_transpose(h5, self.gen_W6, output_shape=output_shape_l6, strides=[1,1,1,1])

        #x = tf.nn.tanh(h5)
        x = tf.nn.sigmoid(h5)
        return latent_codes, x
    def sample_latent_encoder(self, image, batch_size):
        h1 = tf.nn.relu( batchnormalize(tf.nn.conv2d( image, self.aux_q_W1, strides=[1,2,2,1], padding='SAME' )))
        
        h2 = tf.nn.relu( batchnormalize( tf.nn.conv2d( h1, self.aux_q_W2, strides=[1,1,1,1], padding='SAME')) )
        
        h3 = tf.nn.relu( batchnormalize( tf.nn.conv2d( h2, self.aux_q_W3, strides=[1,2,2,1], padding='SAME')) )
        
        h4 = tf.nn.relu( batchnormalize( tf.nn.conv2d( h3, self.aux_q_W4, strides=[1,1,1,1], padding='SAME')) )
