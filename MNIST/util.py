...
def truncate_normal(mu, sigma, size=None):
    lower, upper=-2,2
    rng=truncnorm((lower-mu)/sigma, (upper-mu)/sigma, loc=mu, scale=sigma)
    
    return rng.rvs(size=size)
def mixtures_noise(mean,num_mixture,num_sample, dim):
    #vec=np.random.normal(0,1,size=(num_sample,dim))
    vec=truncate_normal(0,1,size=(num_sample,dim))
    component_list=np.random.randint(num_mixture,size=[num_sample])
    vec=vec+mean[component_list]
    return vec, component_list
